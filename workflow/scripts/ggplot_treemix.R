## treemix.R
## source https://github.com/andrewparkermorgan/popcorn
## 
...
#' 
#' @export
plot_treemix <- function(obj, plot.nodes = TRUE, plot.migration = TRUE, branch.colour = "grey", branch.width = 0.5, label = TRUE, pop.df = NULL, .col.pal, ...) {
  ...
  if (length(pop.df)>0)
    stuff$tips<-left_join(stuff$tips, pop.df, by = c("pop"="sample"))
  ...
  if (plot.migration && nrow(subset(stuff$edges, type == "MIG"))) {
    p <- p + ggplot2::geom_curve(data = subset(stuff$edges, type == "MIG"),
      ...
      ggplot2::geom_point(data = subset(stuff$edges, type == "MIG"),
                          ggplot2::aes(x = from.x, y = from.y, 
                                       colour = weight)) +
      ggplot2::scale_colour_gradient2("migration weight", high = "red", mid = "yellow")
  }
  
  if (label && length(pop.df)>0)
    p <- p +
    ggnewscale::new_scale_color() +
    ggplot2::geom_text(data = stuff$tips, ggplot2::aes(x = xo, y = y, label = pop, color = group.label), hjust = 0)+
    scale_color_manual("population",values = .col.pal)
