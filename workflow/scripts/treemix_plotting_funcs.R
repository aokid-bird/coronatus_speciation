# This is originally a script provided by the TreeMix source by Pickrell and Pritchard.
# This is pushed to git just for the functionality test.
...
calcVarExplain <- function(stem) {
  ...
  WHat <- read.table(gzfile(paste(stem, ".cov.gz", sep = "")),
    as.is = T,
    head = T, quote = "", comment.char = ""
  )
  W <- read.table(gzfile(paste(stem, ".modelcov.gz", sep = "")),
    as.is = T,
    head = T, quote = "", comment.char = ""
  )
  ...
  se <- read.table(gzfile(paste(stem, ".covse.gz", sep = "")),
    as.is = T,
    head = T, quote = "", comment.char = ""
  )
